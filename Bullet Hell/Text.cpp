...
{
	return m_font->as<aie::Font>()->getStringWidth(m_text.c_str()) / 2;
}

float Text::getHalfHeight()
{
	return m_font->as<aie::Font>()->getStringHeight(m_text.c_str()) / 2;
}

void Text::onDraw(aie::Renderer2D* renderer)
{
	renderer->setRenderColour(m_colour.x, m_colour.y, m_colour.z, m_colour.w);
	renderer->drawText(m_font->as<aie::Font>(), m_text.c_str(),
